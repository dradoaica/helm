...
| authentication.igniteUserNewPassword | string | `""` | New password for the ignite user (defaults to ignite when empty) |
| autoscaling.create | bool | `false` | Create HorizontalPodAutoscaler |
...
| dataStorage.config | string | `""` | Additional config for org.apache.ignite.configuration.DataStorageConfiguration class |
| env | object | `{"EXTRA_JVM_OPTS":"","EXTRA_OPTION_LIBS":""}` | Application-specific environment variables |
...
| ingress.create | bool | `false` | Create Ingress for external HTTP access to REST API |
...
| memory.offHeapMb | int | `2048` | Off-heap memory limit in MB. Configures DataRegionConfiguration MaxSize. |
| memory.onHeapMb | string | `""` | On-heap memory limit in MB. If set, -Xms will be value/2 and -Xmx will be value. If not set, defaults to 1024. |
...
| persistence.storageVolume.annotations | object | `{}` | Annotations to add to the storage PersistentVolumeClaim (PVC) |
| persistence.storageVolume.labels | object | `{}` | Labels to add to the storage PersistentVolumeClaim (PVC) |
...
| persistence.walVolume.annotations | object | `{}` | Annotations to add to the WAL PersistentVolumeClaim (PVC) |
| persistence.walVolume.labels | object | `{}` | Labels to add to the WAL PersistentVolumeClaim (PVC) |
...
| resources | object | `{"limits":{"cpu":"2","memory":"5Gi"},"requests":{"cpu":"1","memory":"2Gi"}}` | Main container requests/limits  memory calculation: onHeapMb (JVM Heap max size)   + maxMetaspaceSize (JVM Metaspace max size)   + offHeapMb (default region max size)   + offHeapMb * 0.3 (indexes also require memory; basic use cases will add a 30% increase)   + MIN(256MB, offHeapMb) (offHeapMb < 1 GB)     || offHeapMb/4 (offHeapMb between 1 GB and 8 GB)     || 2GB (offHeapMb > 8 GB)       (default region checkpointing buffer size)   + 100MB (sysMemPlc region max size)   + 100MB (metastoreMemPlc region max size)   + 100MB (TxLog region max size)   + 100MB (volatileDsMemPlc region max size) |
...
| ssl.keyStoreFilePath | string | `""` | Filesystem path to the keystore that stores Ignite node certificates |
...
| ssl.trustStoreFilePath | string | `""` | Filesystem path to the truststore used to validate connecting clients |

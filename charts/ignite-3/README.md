...
| dataStorage.engine | string | `"aipersist"` | Storage engine to use (aipersist for durable, aimem for in-memory-only deployments) |
| env | object | `{"EXTRA_JVM_OPTS":""}` | Application-specific environment variables |
...
| image.tag | string | `"3.1.0"` | Main container image tag |
...
| memory.maxMetaspaceSizePercentage | int | `5` | Percentage of memory limit to use for JVM Metaspace. |
| memory.offHeapPercentage | int | `60` | Percentage of memory limit to use for default storage profile. |
| memory.onHeapPercentage | int | `30` | Percentage of memory limit to use for JVM Heap (JVM_MAX_MEM). JVM_MIN_MEM will be set to JVM_MAX_MEM/2. |
...
| resources | object | `{"limits":{"cpu":"2","memory":"5Gi"},"requests":{"cpu":"1","memory":"2Gi"}}` | Main container requests/limits |
...
| ssl.clientConnector.enabled | bool | `false` | Enable SSL for the client connector endpoint |
